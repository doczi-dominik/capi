...
function lib.newVerticalContainer(options, style)
	...
	function c.computeLayout(x, y, w, h)
		c._computeLayout(x, y, w, h, function()
			local cy = c.y
			for i = 1, #c.children do
				...
				c.children[i].computeLayout(
					...
					cy + c.padding[2],
					c.w - c.padding[3] * 2,
					math.min(ch, c.h - cy + c.y) - c.padding[4] * 2
				)
	...
	function c.mouseInput(x, y, button, type)
		...
		for i = 1, #c.children do
			startPos = startPos
				+ math.min(c.children[i].sizeFactor * c.h, c.h - startPos + c.y)
				+ c.children[i].margin[2]
...
function lib.newHorizontalContainer(options, style)
	...
	function c.mouseInput(x, y, button, type)
		...
		for i = 1, #c.children do
			startPos = startPos
				+ math.min(c.children[i].sizeFactor * c.w, c.w - startPos + c.x)
				+ c.children[i].margin[1]
...
function lib.newTextInput(options, style)
	...
	function c.draw()
		...
		if c.color ~= nil and c.text ~= "" then
			...
			LG.print(
				c.placeholder,
				c.x + c.w / 2 - lib.default_font:getWidth(c.placeholder) / 2,
				c.y + c.h / 2 - lib.default_font:getHeight() / 2
			)
		end
...
function lib.newListContainer(options, style)
	...
	function c.computeChildren()
		local itemCount = 0
		for i = 1, #c.items do
			c.items[i].computeLayout(
				c.x,
				c.y + (c.topBarSize * lib.getScale()) + (c.item_height * (i - 1)) + c.scrollPos,
				c.w,
				c.item_height
			)
			itemCount = i
